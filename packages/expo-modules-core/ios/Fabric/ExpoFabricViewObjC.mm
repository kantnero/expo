...
/**
 Converts a Foundation event payload to a `jsi::Value`.
 Handles the common types that appear in view event payloads.
 */
jsi::Value convertEventPayloadToJSIValue(jsi::Runtime &runtime, id value)
{
  ...
  if ([value isKindOfClass:[NSNumber class]]) {
    if ([value isKindOfClass:[@YES class]]) {
      return jsi::Value([value boolValue]);
    }
  ...
  if ([value isKindOfClass:[NSDictionary class]]) {
    ...
    for (NSString *key in value) {
      obj.setProperty(runtime, [key UTF8String], convertEventPayloadToJSIValue(runtime, value[key]));
    }
    return jsi::Value(runtime, std::move(obj));
  }
  if ([value isKindOfClass:[NSArray class]]) {
    NSArray *array = value;
    jsi::Array arr(runtime, array.count);
    for (size_t i = 0; i < array.count; i++) {
      arr.setValueAtIndex(runtime, i, convertEventPayloadToJSIValue(runtime, array[i]));
    }
    return jsi::Value(runtime, std::move(arr));
  }
